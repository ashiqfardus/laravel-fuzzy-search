...
- suggest() completes the last word from the BM25 dictionary (scoped to the model's postings, ordered by document count) when the model is indexed and the base query carries no `where()` or scope (SoftDeletes aside), falling back to the table scan otherwise; suggestFrom('index'|'table') overrides.
...
- `paginate()` applies one page-size rule on every path: `perPage` is clamped to `max_candidates` (default 1000). The BM25 path used to clamp at a hard-coded 100 while the LIKE and extended paths clamped at nothing, so `paginate(200)` returned 200 rows without `useInvertedIndex()` and 100 with it. `paginate(200)` now returns 200 everywhere; a `perPage` above `max_candidates` (previously unbounded on the LIKE path) now returns `max_candidates`, and one below 1 returns a one-row page (`paginate(0)` used to throw `DivisionByZeroError`). `simplePaginate()` is clamped the same way: it accepted any page size, so on the index path `simplePaginate($request->per_page)` hydrated that many models. `take()`/`limit()` and `FederatedSearch::paginate()` are not clamped.
...
- BM25 typo expansion draws its candidates from the searched model's own dictionary terms only. The candidate pool used to be filled from every indexed model's terms, so another model's popular terms could crowd this model's close neighbours out, and rankings on the typo-tolerant index path can change.
...
- **`suggest()` no longer runs inside the previous search.** It clones the builder's query, which `get()` had already filled with the search conditions, so the Livewire recipe in `docs/integrations.md` — suggest on the same builder when the page came back empty — could only ever return `[]`. Constraints the caller applied (`where()`, scopes) still scope the suggestions; the search's own conditions no longer do. `didYouMean()` reads the dictionary rather than the query and was never affected.
...
- **Invalid UTF-8 in a search term (present since 1.x).** A crafted request such as `?q=john%C3` reached the bind parameters as-is: PostgreSQL (SQLSTATE 22021) and SQL Server (IMSSP, "translating string ... to UCS-2") rejected it, so the search was a 500 there, while SQLite and MySQL searched the raw bytes. Invalid UTF-8 byte sequences are now dropped from the term wherever it enters the package — `search()`, `extended()`/`searchBoolean()`, the `whereFuzzy`/`orWhereFuzzy`/`whereFuzzyMultiple`/`orderByFuzzy`/`fuzzySearch` macros and the `Fuzzy` trait scopes, `FuzzySearch::tableSearch()`, Filament global search, `FederatedSearch`, `FuzzySearch::on()`, the Scout engine and the analytics recorder — so `jo%C3hn` searches `john` and behaviour is identical on all five databases. Valid characters are untouched, and the bytes are dropped rather than replaced (a `?` would be a literal in a LIKE pattern).
  - A term made only of invalid bytes (`?q=%FF`) gets past an app's `filled('q')` check but cleans to nothing. It now matches nothing, like a term below `min_search_length`. `get()`, `first()`, `simplePaginate()`, `paginate()` (total 0), `count()`, `getFacets()`, `FederatedSearch` and `FuzzySearch::on()` return no rows on every database. It never throws `EmptySearchTermException` and never returns every row. A genuinely empty `''` is unchanged.
  - `extended()`/`searchBoolean()` split words on ASCII whitespace only. `ctype_space()` follows the locale: under a UTF-8 `LC_CTYPE` on macOS/BSD it counted byte 0xA0 as a space, which cut characters such as `à`, `ঠ` and `丠` in half. A valid query then sent invalid UTF-8 to the database (PHP 8.1/8.2) or searched for a literal `?` (8.3+).
- **`didYouMean()` offered every indexed model's terms.** It read the whole cross-model dictionary, and `FuzzySearchCollection` publishes it as `meta.suggestions` on every empty page, so a public product search disclosed other models' tokens (users' names, email domains). It now offers only terms posted under the searched model, and returns `[]` for a builder with no Eloquent model (or `useInvertedIndex(Model::class)`).
- **`suggest()` and `didYouMean()` ignored `where()` and global scopes on indexed models.** Dictionary completions and alternatives were scoped to the model only, so on a multi-tenant indexed model one tenant was offered another tenant's terms, while `docs/integrations.md` said such constraints still applied. `suggest()` in its default `'auto'` mode now uses the table scan, which honours them, whenever the base query is constrained. `didYouMean()` keeps only terms posted for a row the constrained query can see, checked by primary key against the postings (no table scan). The `SoftDeletes` scope is not counted, because the index already drops a deleted row's terms. `suggestFrom('index')` stays model-wide and is documented as ignoring these constraints.
