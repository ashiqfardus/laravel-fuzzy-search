...
- **Auto-detection could index secrets and hidden attributes.** A model without `$searchable['columns']` has its columns auto-detected, and since this release indexed. Detection ignored `$hidden` and `$visible`, and its last fallback, the first string-like column, could pick `password`, `remember_token` or `two_factor_secret` (the LIKE path has searched such a pick since 2.0). The indexer read each column through `getAttribute()`, so a get accessor that decrypts a column (`getBioAttribute()` returning `decrypt($value)`, or `Attribute::make(get: …)`) wrote the plaintext to `fuzzy_index_terms`, where `suggest()` and `didYouMean()` served it. Detection now skips hidden columns and `password`, `remember_token`, `two_factor_secret`, `two_factor_recovery_codes` and `api_token` in every branch, and an auto-detected column is indexed, and its `*_metaphone` shadow column filled, from its stored value: the value the LIKE search matches. Declared columns are unchanged, and declaring a column is what opts into its accessor.
