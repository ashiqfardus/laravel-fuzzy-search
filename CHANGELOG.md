...
- **SQL Server: two index writes adding the same new word at the same moment failed the second (present in 2.0).** SQL Server's `MERGE` checks for the word without locking it, so both writes inserted it and the second failed with a duplicate-key error (2627), which no retry covered. <!-- r7-b-round5 --> While `XACT_ABORT` is off (SQL Server's default), the write now runs the statement again and updates the row the other write inserted; with it on, SQL Server has already rolled the whole transaction back, so the write fails and rolls back whole instead of going on without its start, and the queue retries it. The same holds for a model's first document row and meta row.
