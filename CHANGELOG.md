...
- **`extended()` / `searchBoolean()`: a `!` inside or at the end of a word is part of the word (present since 2.0.0).** The lexer ended a word at any `!` and read what followed as a NOT term, so `yahoo!mail` meant `yahoo !mail` and excluded the very row it named, and `'wow!` searched for `wow`. `!` now negates only at the start of a term, as `~` and `field:` are operators only there. This changes the meaning of `a!b`, and of 2.0.0's `=John!!!`, whose trailing `!` characters are now part of the term; write `a !b` for the old meaning (see the upgrade guide).
