...
- **A literal `%` or `_` in a search term matched nothing on SQLite and SQL Server (present since 1.x), and a literal `\` was lost on MySQL, MariaDB and PostgreSQL.** The package escaped `%` and `_` with a backslash, which only MySQL, MariaDB and PostgreSQL read as the LIKE escape character. SQLite and SQL Server have none by default, so the backslash became one more character the value had to contain: `search('50%')` and `search('snake_case')->using('simple')` returned nothing, and so did `extended()`, the query-builder macros, `suggest()`'s table scan, the relevance ordering and, on SQL Server, the dictionary's prefix lookup. Every LIKE now carries `ESCAPE '\'` on SQLite and SQL Server, and on SQL Server a `[` in a term is escaped too: it opened a character class, so `[draft]` matched any value with a d, r, a, f or t. A backslash in a term is now escaped as well. MySQL, MariaDB and PostgreSQL read it as an escape, so `back\slash` matched `backslash` and not itself; it now matches literally on all five databases. For a term without `\`, `%`, `_` or `[`, the SQL and bindings on MySQL, MariaDB and PostgreSQL are unchanged.
