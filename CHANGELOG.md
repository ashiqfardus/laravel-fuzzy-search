...
- Extended syntax: `~word` and `field:` are operators, `!` negates only at the start of a token, `""` is skipped, a `!` term keeps rows with NULL columns, and a query may have exactly `query.max_tokens` tokens (it threw at the limit).
- `FederatedSearch` searches each model with its own configuration, orders ties deterministically and counts matches, not page rows. Scout `orderBy()` orders, `hybrid()` throws, and the Scout engine's `update()` writes a collection in one transaction.
- A backslash in a search term is literal on MySQL, MariaDB and PostgreSQL.
- Config keys documented as reserved are read (`scoring.*`, `highlighting.*`, `performance.max_patterns`, `unicode.normalize`), and so is the global `synonyms` key; three unread keys are removed; `debounce()`, `locale()` and `minMatchLength()` are deprecated. `ignoreStopWords('en')` prefers the shorter configured list.
...
- **The global `synonyms` config key was never read.** `config/fuzzy-search.php` documented it as global synonym mappings, but only `$searchable['synonyms']` and `withSynonyms()` reached a search. Its mappings now apply to every search as defaults, and a model's and a query's synonyms are merged on top of them.
