...
- suggest()'s table scan named its columns bare, so a joined table with a column of the same name made the query ambiguous (a SQL error); on an Eloquent query they are now qualified with the model's table.
