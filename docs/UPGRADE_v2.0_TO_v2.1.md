...
- **A model on the `fuzzy-search` driver needs the package's `Searchable` trait.** With Scout's trait alone it was indexed and searched as nothing; indexing and searching it now throw `LogicException`, naming the trait. (2.0.1 threw on indexing, and returned nothing on search.) Add `use Ashiqfardus\LaravelFuzzySearch\Traits\Searchable;` beside Scout's trait, as in the [recipe](integrations.md#usage).
