...
- **`fallback()` actually runs now.** Previously the fallback algorithms you registered were
  ...
  each fallback runs in order until one returns results. A fallback runs only when the primary matches nothing at all; a `simplePaginate()` or `skip()` page past the primary's matches stays empty.
