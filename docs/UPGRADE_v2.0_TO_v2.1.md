...
- **Another new migration to run.** `2026_09_20_000001_widen_model_id_on_fuzzy_index_tables` widens `model_id` to 191 characters on `fuzzy_index_postings` and `fuzzy_index_documents`, so string keys longer than a UUID can be indexed. On MySQL and MariaDB it copies both tables (`ALGORITHM=COPY`: under utf8mb4 the column's length prefix grows from one byte to two, which InnoDB cannot change in place), and writes to each table wait until its copy finishes: roughly 10 seconds per million postings. PostgreSQL and SQL Server widen the column in place, as a metadata change; SQL Server also drops and recreates the documents primary key, which rebuilds `fuzzy_index_documents` (one row per indexed record). On a large index, run it in a maintenance window with the indexing workers paused. Rolling it back deletes index rows whose key is longer than 36 characters.
