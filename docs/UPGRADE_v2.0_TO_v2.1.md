...
- **The `synonyms` config key is read.** 2.0 ignored it. If your published config lists synonyms, every `SearchBuilder` search now uses them (not the Scout engine, `FuzzySearch::on()` or the query-builder macros), merged under a model's `$searchable['synonyms']` and a query's `withSynonyms()`. Empty the key to keep 2.0's results.
...
| `synonyms` | Default synonyms (lower-case word => its synonyms) for every `SearchBuilder` search, as if `withSynonyms()` were called first; a model's `$searchable['synonyms']` and a query's `withSynonyms()` are merged on top | Ignored — only `$searchable['synonyms']` and `withSynonyms()` reached a search | `[]` |
...
`synonyms` is empty; mappings you put there now apply to every `SearchBuilder` search.
