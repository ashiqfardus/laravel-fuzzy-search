...
- **New migration to run.** `fuzzy_index_postings` gained a `column_name` column — run `php artisan migrate`. Existing rows are backfilled with `''` and keep scoring exactly as before (weight 1); nothing breaks if you don't rebuild.
...
- **The Scout engine is unchanged** — it does not read `fuzzy_index_postings` and does not apply column weights, same as v2.0.
