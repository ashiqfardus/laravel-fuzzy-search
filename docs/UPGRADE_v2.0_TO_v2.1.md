...
- See [Persisted Search Analytics](analytics.md#persisted-search-analytics) in the README for the opt-in `fuzzy_search_logs` recording, the `SearchAnalytics` query API and the two `fuzzy-search:analytics*` commands — none of this runs unless you enable `analytics.enabled`.
...
- See [Filament Integration](integrations.md#filament-integration), [JSON API Resources](integrations.md#json-api-resources) and [Livewire Recipe](integrations.md#livewire-recipe) in the README.
