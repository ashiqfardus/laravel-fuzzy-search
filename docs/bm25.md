...
`orderBy()` on the builder replaces the BM25 rank order. The page is then the first matches in that order, ties broken by the primary key ascending. The query keeps its select list, so you can order by a `withCount()` or `selectRaw()` alias. When there are up to `bm25.candidate_chunk` matches (200 by default), the query is restricted to their ids. Past that, the package reads the constrained table in your order, 1,000 rows per query, until it has `offset + perPage` matches. That costs one sorted page query per 1,000 rows it passes: cheap when the matches come early in the order, and up to the whole constrained table when they come late. Under `orderBy()`, `_score` is normalised against the best-scoring row in the window up to the end of the page, so it is not comparable across pages; without `orderBy()` it is anchored at rank 1. A model that overrides `getSearchScore()` has its raw BM25 score passed through it before normalisation, and it is ranked over the first `max_candidates` matches. Every terminal (`get()`, `first()`, `paginate()`, `simplePaginate()`, `count()`) caps the term at `query.max_term_length`. An empty term throws `EmptySearchTermException` on every terminal, unless `allow_empty_search` is on.
...
<!-- r7-b-round5 -->
Run `fuzzy-search:flush`, `fuzzy-search:clear` and `rebuild --fresh` (which flushes first) while nothing is indexing any model: pause the queue workers that index, or run it with `indexing.enabled` off. A flush gives back the flushed model's share of each word's `doc_count`, then deletes every word no model's index uses any more, whichever model it came from. A write for the flushed model that commits during the flush can leave a word that other models share with a `doc_count` that is too high. That count only orders `didYouMean()`, `suggest()` and the typo and prefix expansions, not BM25 scores. `fuzzy-search:clear --all` followed by a rebuild of each model resets it. A write for another model that reuses a word the flush is deleting is not safe either. On MySQL/MariaDB the write inserts the word again, but the two can deadlock (the write retries, and the flush can be run again). On PostgreSQL and SQL Server the flush can delete the word after the write commits, together with the write's new posting, so that row is not found by that word until it is indexed again.
