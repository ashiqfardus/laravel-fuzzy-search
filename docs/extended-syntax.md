...
`field:word` limits a term to one searchable column: a direct column, a table-qualified column matched by its bare name (`users.name` answers to `name:`), or a relation column declared in `searchIn()` / `$searchable['columns']` (`author.name:smith`). Any operator can follow the colon — `email:^admin`, `name:~jonh`, `name:"john doe"` — except `!`: negation goes before the field (`!name:bob`), and `name:!bob` throws `QuerySyntaxException` saying so. An unknown field throws `QuerySyntaxException` listing the searchable fields the model shows, by their bare names: a column in `$hidden` (or left out of a non-empty `$visible`), and a relation column its related model hides, are not listed; `field:` with nothing after the colon throws too, and so does a bare name that matches two searchable columns (`users.name` and `profiles.name`) — qualify it, `users.name:john`.

A field scope reaches every searchable column, a hidden one included, as a free-text term does: `email:john@` finds rows by `email` even when the model hides it, although the rows it returns still hide `email` from `toArray()`. So a user who types the query can test a hidden column's contents one field at a time. For untrusted input, such as a public search box, declare only public columns searchable (in `$searchable['columns']`, or as `searchOn()`'s third argument, which replaces the declared list; `searchIn()` adds to it), or check the fields a query names against an allowlist before you pass it to `extended()` or `searchBoolean()`.
